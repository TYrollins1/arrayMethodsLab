...
// CODE HERE
const evenNumbers = mixedNumbers.filter(function(element, wholeArray) {
  return element % 2 === 0 
}, 0)
...
// CODE HERE
const postTaxPrices = prices.map(function(num, arr) {
  return num * (1 + .07)
})

console.log(postTaxPrices)

const postTaxPrice = prices.map(num => num * (1 + .07))
...
const totalPop = populations.reduce((acc, curr) => acc + curr)
...
const myStrong = monstersInYourPocket.filter(element => element.CP >= 200)
...
// CODE HERE
const totals = orders.map(function(num){
  return num.price * (1 + num.tax)
})

const total = orders.map(num => num.price * (1 + num.tax))
